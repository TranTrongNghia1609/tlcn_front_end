...
const ShareMySolutionPage = () => {
  ...
  const solutionTemplate = `# Hướng giải quyết
...
\`\`\``;
    ...
    try {
      ...
      if (submissionId) {
        ...
        const languageMap = {
          ...
          'python3': 'Python3', 'c': 'C', 'csharp': 'C#', 'c#': 'C#',
          'javascript': 'JavaScript', 'js': 'JavaScript',
          'typescript': 'TypeScript', 'ts': 'TypeScript',
          'php': 'PHP', 'swift': 'Swift', 'kotlin': 'Kotlin',
          'dart': 'Dart', 'go': 'Go', 'ruby': 'Ruby',
          'scala': 'Scala', 'rust': 'Rust'
        };

        const lang = languageMap[submissionData.language?.toLowerCase()] || 'C++';
        const codeBlock = `# Code\n\`\`\`${submissionData.language || 'cpp'}\n${submissionData.source || submissionData.code || '// Your code here'}\n\`\`\``;

        const updatedContent = solutionTemplate.replace(
          '# Code\n```cpp\nclass Solution {\npublic:\n    // Your solution here\n};\n```',
          codeBlock
        );
        ...
        setFormData(prev => ({
          ...
          title: `${lang} Solution - ${submissionData.status || 'Accepted'}`
        }));
      } else {
        setFormData(prev => ({ ...prev, content: solutionTemplate }));
      }
    } catch (error) {
      console.error('Load submission error:', error);
      toast.error('Không thể tải submission');
      setFormData(prev => ({ ...prev, content: solutionTemplate }));
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.title.trim()) return toast.error('Nhập tiêu đề');
    if (!formData.content.trim()) return toast.error('Nhập nội dung');
    if (!formData.complexity.time || !formData.complexity.space) return toast.error('Nhập độ phức tạp');

    try {
      setSubmitting(true);
      const response = await solutionService.createSolution({ ...formData, problemShortId: id });
      const newSolutionId = response.data._id;
      
      toast.success('Chia sẻ solution thành công!');
      navigate(`/problemset/problem/${id}/solutions/${newSolutionId}`);
    } catch (err) {
      ...
      <div className="bg-white border-t py-1 sticky top-0 z-10">
        <div className="max-w-full px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-4">
            <Button className="cursor-pointer" variant="ghost" size="sm" onClick={() => navigate(`/problemset/problem/${id}`)}>
              <ArrowLeft className="w-4 h-4 mr-2" /> Back
            </Button>
            <div className="flex items-center gap-3">
              <Lightbulb className="w-7 h-7 text-yellow-500" />
              <h1 className="text-2xl font-bold">Share My Solution</h1>
              {submission && <Badge variant="outline">From #{submission.shortId}</Badge>}
            </div>
          </div>
          <div className="flex gap-3">
            ...
            <Button className="cursor-pointer bg-green-600 hover:bg-green-700" onClick={handleSubmit} disabled={submitting}>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Select value={formData.approach} onValueChange={v => setFormData(p => ({ ...p, approach: v }))}>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                />
                <Button className="cursor-pointer bg-green-600 hover:bg-green-700" type="button" onClick={handleAddTag} disabled={formData.tags.length >= 5}>
...
export default ShareMySolutionPage;