import React, { useState } from 'react';
import OnboardingModal from '../components/auth/OnboardingModel';

const Home = ({isShowOnboarding = false}) => {
  console.log(isShowOnboarding);
  const [showOnboarding, setShowOnboarding] = useState(isShowOnboarding);
  return (
    <>
      <main className="max-w-6xl mx-auto py-6 sm:py-8 px-4">
        <div className="text-center">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-800 mb-3 sm:mb-4">
            Chào mừng bạn đến với ứng dụng của chúng tôi!
          </h2>
          <p className="text-base sm:text-lg md:text-xl text-gray-600 px-4">
            Hãy đăng nhập hoặc đăng ký để bắt đầu sử dụng.
          </p>
        </div>
      </main>
      <OnboardingModal open={showOnboarding} 
        onClose={() => setShowOnboarding(false)} 
      />
    </>
  );
};

export default Home;